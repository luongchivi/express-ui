...
const DealDaily = () => {
    ...
    const fetchDealDaily = async () => {
        const response = await apiGetProducts({pageSize: 1, page: Math.round(Math.random() * 4) + 1, averageRating: 5});
        if (response?.results?.statusCode === 200) {
            setDealDaily(response?.results?.products[0]);
            setHour(24)
            setMinute(59)
            setSecond(59)
        }
    ...
    return (
        <div className="border w-full flex-auto">
            <div className="flex items-center justify-between p-4 w-full">
                <span className="flex-1 flex justify-center"><FaStar color="red" size={20}/></span>
